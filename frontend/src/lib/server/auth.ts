...
// Verify PBKDF2 passwords only (salt:hash format)
// NOTE: For bcrypt passwords, use the async verifyLogin() function instead
function verifyPassword(password: string, storedHash: string): boolean {
  // This function only supports PBKDF2 passwords (salt:hash format)
  // Bcrypt hashes must use the async verifyLogin() function
  if (storedHash.startsWith('$2')) {
    console.error('verifyPassword called with bcrypt hash - use verifyLogin() instead');
    return false;
  }

  // For PBKDF2 passwords (salt:hash format)
...
export function cleanExpiredSessions(): void {
  try {
    const now = new Date().toISOString();
    // Delete sessions where expiresAt < now (already expired)
